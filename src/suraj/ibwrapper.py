...
class IBRelay(EClient, EWrapper):
    ...
    def historicalDataEnd(self, reqId, start, end):
        print('\nHistorical Data Retrieved Printing Data now\n')

        print(self.df.head())

        #Print this dataframe based on req id
        self.df.to_csv(str(reqId) + '.csv')
    ...
    def tickByTickBidAsk(self, reqId, time, bidPrice, askPrice,
        ...
        #print(int(t.time() / 60))
        print("Time:" + str(int(t.time())) + "  --- " + str(time))
...
class IBWrapper(object):
    ...
    def give_historical_data(self, inst, reqid, duration='6 M', barsize='5 mins'):
        obj_return = self.local_app.reqHistoricalData(reqId=reqid,
                                         contract=inst,
                                         endDateTime='',
    ...
    def initialise(self, combo_list):
        for combo in combo_list:
            for instrument in combo:
                ...
                self.reqid += 1
                #Add Req Id and Instrument to a list
                f = open("instrument_list.txt", "w")
                f.write(str(self.reqid) + ": " + str(instrument))
                f.close()
